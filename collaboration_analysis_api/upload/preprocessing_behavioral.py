...
def compute_centralities(df):
    ...
    for i in range(len(df)):
        ...
        if prev_speaker != next_speaker and df.iloc[i]['Text'].strip() != '':
            combined_row = combined_dataset[(combined_dataset['project'] == df['project'].iloc[i]) & (combined_dataset['meeting_number'] == df['meeting_number'].iloc[i]) & (combined_dataset['speaker_number'] == int(prev_speaker.split('_')[1])) & (combined_dataset['next_speaker_id'] == int(next_speaker.split('_')[1]))]
    ...
    if len(G) == 0:
        ...
        try:
            eigenvector_cent = eigenvector_centrality(G, max_iter=2000, weight='weight')
