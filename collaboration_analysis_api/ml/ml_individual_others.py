...
def initialize_individual_others_ml_app(dash_app_instance, dataset_instance):
    ...
    dash_app.layout.children.append(html.Div([
        html.H1("ML Models for Individual Others Collaboration Score"),
        html.Div(id='ml-individual-others-output'),
        html.Button('Build Model for Individual Others Collaboration Score', id='build-ml-individual-others', n_clicks=0),
        dcc.Loading(id="loading-individual-others", type="default", children=html.Div(id="loading-individual-others-output"))
    ]))

    @dash_app.callback(
        Output('loading-individual-others-output', 'children'),
        [Input('build-ml-individual-others', 'n_clicks')]
    )
    def build_model(n_clicks):
        if n_clicks > 0:
            return build_individual_others_model()
        return ""

def build_individual_others_model():
    global dataset
    ...
    target = dataset['individual_others_collaboration_score']
    ...
    def predict_and_evaluate(best_reg_model_info, X_train, y_train, X_test, y_test):
        ...
        plt.savefig('actual_vs_predicted_individual_others.png')
        plt.show()
    ...
    if hasattr(best_reg_model_info['model_object'].named_steps['model'], 'feature_importances_'):
        ...
        plt.savefig('feature_importance_individual_others.png')
        plt.show()
    ...
    return html.Div([
        ...
        html.Img(src='data:image/png;base64,' + base64.b64encode(open('feature_importance_individual_others.png', 'rb').read()).decode()),
        html.H4('Actual vs Predicted:'),
        html.Img(src='data:image/png;base64,' + base64.b64encode(open('actual_vs_predicted_individual_others.png', 'rb').read()).decode()),
