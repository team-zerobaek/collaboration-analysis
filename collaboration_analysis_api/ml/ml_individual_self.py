...
def initialize_individual_self_ml_app(dash_app_instance, dataset_instance):
    ...
    # Include only self-interactions
    ...
    dash_app.layout.children.append(html.Div(id='self', children=[
        ...
        html.Button('Run Actual Model',
                    id='run-actual-individual-self', n_clicks=0),
        dcc.Loading(id="loading-individual-self", type="default",
                    children=html.Div(id="loading-output-individual-self")),
        html.Details([
    ...
    @dash_app.callback(
        Output('loading-output-individual-self', 'children'),
        [Input('run-dummy-individual-self', 'n_clicks'),
         Input('run-actual-individual-self', 'n_clicks')]
    )
    def build_model(dummy_clicks, actual_clicks):
        ...
        if button_id == 'run-dummy-individual-self':
            return build_dummy_model()
        elif button_id == 'run-actual-individual-self':
